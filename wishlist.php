...
?>
    ...
    <div class="page-header">
        <h1>Избранное</h1>
        <p class="small-text">Сохранённые товары</p>
    </div>
    ...
    <div class="product-list">
        <?php foreach ($wishlistItems as $item): ?>
            ...
            <div class="product-card">
                ...
                <p><?= e(tdb($item, 'short_description')) ?></p>
                <div class="price">€<?= number_format((float)$item['price'], 2) ?></div>

                <div style="display:flex; gap:10px; flex-wrap:wrap;">
                    <a class="btn" href="/3d_print_shop/product.php?id=<?= (int)$item['id'] ?>">Открыть</a>
                    <a class="btn btn-secondary" href="/3d_print_shop/add_to_cart.php?id=<?= (int)$item['id'] ?>">В корзину</a>
                    <a class="btn btn-danger" href="/3d_print_shop/toggle_wishlist.php?id=<?= (int)$item['id'] ?>">Убрать</a>
                </div>
            </div>
        <?php endforeach; ?>
    </div>
